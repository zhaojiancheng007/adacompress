...
def get_transformations(db_name, flagvals, train_scale, test_scale):
    """Return transformations for training and evaluation"""
    ...
    if db_name == 'NYUD':
        ...
        raise ValueError(f'Invalid train db name: {db_name}')

    transforms_tr.extend([
        tr.FixedResize(
            resolutions={x: tuple(train_scale) for x in flagvals},
            flagvals=flagvals
        ),
        tr.AddIgnoreRegions(),
        tr.ToTensor(),
    ...
    transforms_ts = [
        tr.FixedResize(
            resolutions={x: tuple(test_scale) for x in flagvals},
            flagvals=flagvals
        ),
        tr.AddIgnoreRegions(),
        tr.ToTensor(),
