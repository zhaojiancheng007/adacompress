...
class Mlp(nn.Module):
    ...
    def forward(self, x, x_tasks=None, task=None):
        x, fc1_lora_tasks = self.fc1(x, x_tasks, task)
        x = self.act(x)
        x = self.drop(x)
        if fc1_lora_tasks is not None:
            for t in task:
                fc1_lora_tasks[t] = self.act(fc1_lora_tasks[t])
                fc1_lora_tasks[t] = self.drop(fc1_lora_tasks[t])
        x, fc2_lora_tasks = self.fc2(x, fc1_lora_tasks, task)
        x = self.drop(x)
        if fc2_lora_tasks is not None:
            for t in task:
                fc2_lora_tasks[t] = self.drop(fc2_lora_tasks[t])
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, task=None):
        ...
        x, x_proj_lora_tasks = self.proj(x,task=task)
        x = self.proj_drop(x)
        if x_proj_lora_tasks is not None:
            for t in task:
                x_proj_lora_tasks[t] = self.proj_drop(
                    x_proj_lora_tasks[t])
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, task=None):
        ...
        attn_windows, attn_windows_lora_tasks = self.attn(
            x_windows, mask=self.attn_mask, task=task)
        ...
        if attn_windows_lora_tasks is not None:
            for t in task:
                attn_windows_lora_tasks[t] = attn_windows_lora_tasks[t].view(
                    -1, self.window_size, self.window_size, C)
                attn_windows_lora_tasks[t] = window_reverse(
                    attn_windows_lora_tasks[t], self.window_size, H, W)
                if self.shift_size > 0:
                    attn_windows_lora_tasks[t] = torch.roll(attn_windows_lora_tasks[t], shifts=(
                        self.shift_size, self.shift_size), dims=(1, 2))
                attn_windows_lora_tasks[t] = attn_windows_lora_tasks[t].view(
                    B, H * W, C)
                attn_windows_lora_tasks[t] = shortcut + \
                    self.drop_path(attn_windows_lora_tasks[t])
        x = x.view(B, H * W, C)
        x = shortcut + self.drop_path(x)

        # FFN
        mlp_result, mlp_lora_tasks = self.mlp(
            self.norm2(x), {t: self.norm2(attn_windows_lora_tasks[t]) for t in task} if attn_windows_lora_tasks is not None else None,
            task)
        ...
        if mlp_lora_tasks is None:
            ...
            if attn_windows_lora_tasks is None:
                for t in task:
                    mlp_lora_tasks[t] = self.drop_path(mlp_lora_tasks[t])
            else:
                for t in task:
                    mlp_lora_tasks[t] = attn_windows_lora_tasks[t] + \
                        self.drop_path(mlp_lora_tasks[t])
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, task=None):
        ...
        for blk in self.blocks:
            x, tasks_lora = blk(x, task=task)
        if self.downsample is not None:
            x = self.downsample(x)
            if tasks_lora is not None:
                # for task in self.tasks:
                for t in task:
                    tasks_lora[t] = self.downsample(tasks_lora[t])
        return x, tasks_lora
...
class SwinTransformerMTLoRA(nn.Module):
    ...
    def forward_features(self, x, return_stages=False, flatten_ft=False, task=None):
        ...
        for layer in self.layers:
            x, tasks_lora = layer(x,task=task)
    ...
    def forward(self, x, return_stages=False, flatten_ft=False, task=None):
        x = self.forward_features(x, return_stages, flatten_ft, task=task)
        x = self.head(x)
        return x
