...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        # seg_mse = self.mse(output["semseg"], target)
        # human_mse = self.mse(output["human_parts"], target)
        # sal_mse = self.mse(output["sal"], target)
        # normal_mse = self.mse(output["normals"], target)
        # out["mse_loss"] = (seg_mse + human_mse + sal_mse + normal_mse) / 4.0
